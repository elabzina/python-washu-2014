...
dept1.towns.append(c)
  #1
for town in session.query(Town).join(Department).filter(Town.population>100).order_by(Town.dept_id, Department.id):
    print town.name, town.population

    #2

##m = sqlalchemy.MetaData(session)
#m.drop_all()

for town, dist in session.query(Town,Distance).query(Town.id==Distance.ta OR (Town.population<80000).order_by(Town.id):
    print dist.distance
